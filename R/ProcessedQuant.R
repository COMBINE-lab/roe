...
methods::setMethod("sce", "ProcessedQuant", function(x) x@sce)
...
#' \href{https://github.com/COMBINE-lab/10x-requant}{10x-requant} project.
#' To initialize a ProcessedQuant class, run \code{ProcessedQuant(dataset_id)} 
#' where \code{dataset_id} is the id of an available dataset.
#' Run \code{print_available_datasets()} for obtaining the id of all available
#' datasets.
#' The instantiated ProcessedQuant object needs to go through some function calls,
#' i.e, \code{fetch_quant()}, \code{decompress_quant()} and \code{load_quant()} to
#'  become a complete ProcessedQuant class, in which the \code{sce} slot contains the 
#' \code{SingleCellExperiment} object of the quantification result of the dataset.
...
#' running \code{init_processed_quant(dataset_id)}
...
check_validity <- function(processed_quant) {
    if (any(c(
        ...
        identical(processed_quant@feature_barcode, character(0)),
        identical(processed_quant@library_csv, character(0)),
